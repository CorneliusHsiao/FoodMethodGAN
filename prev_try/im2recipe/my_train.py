...
from new_data_loader import ImageLoader 
from new_args import get_parser
from model import im2recipe
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        ImageLoader(opts.img_path,
            ...
            transforms.RandomCrop(112), # random crop within the center crop 
    ...
    val_loader = torch.utils.data.DataLoader(
        ImageLoader(opts.img_path,
            transforms.Compose([
            transforms.Scale(128), # rescale the image keeping the original aspect ratio
            transforms.CenterCrop(112), # we get only the center of that rescaled
            transforms.ToTensor(),
            normalize,
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    print('Epoch: {0}\t'
          ...
          'vision ({visionLR}) - recipe ({recipeLR})\t'
          'time elapsed ({time:.4f})'.format(
           epoch, loss=cos_losses, visionLR=optimizer.param_groups[1]['lr'],
           recipeLR=optimizer.param_groups[0]['lr'],time=time.time()-epoch_start))            
...
def validate(val_loader, model, criterion):
    ...
    print('Valid Loss ({loss.avg:.4f})\t'
          'time elapsed ({time:.4f})'.format(
           loss=cos_losses,time=time.time()-epoch_start))  
