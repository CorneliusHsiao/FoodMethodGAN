...
if(params['dataset'] == 'MNIST'):
    ...
    from models.mnist_model import Generator, Discriminator, DHead, QHead
elif(params['dataset'] == 'Food'):
    from models.food_model import Generator, Discriminator, DHead, QHead
...
# dataloader returns an image (real) and feature used to generate fake as condition
...
for epoch in range(params['num_epochs']):
    ...
    ######################### change data, _ to img, feat, _ if necessary #########################
    for i, (data, feat, _) in enumerate(dataloader, 0):
        # Get batch size
        b_size = data.size(0)
        # Transfer data tensor to GPU/CPU (device)
        real_data = data.to(device)
        fake_feat = feat.to(device)
        ...
        ######################### change noise to fake_feat as condition #########################
        ##################### does fake_feat be the same shape as image? #########################
        fake_data = netG(fake_feat)
